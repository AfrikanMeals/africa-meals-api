...
export class StoreService {
  ...
  /** Résumé boutique pour l'écran vendeur (statut + fiche éditable si PENDING/REVISION/ACTIVE propriétaire). Messages via `findMyStoreMessages` + WS. */
  async findMyStoreSummary(user: UserModel, storeId?: string) {
    ...
    const vendorRouting =
      await this._subscriptionsService.resolveVendorRoutingForStore(targetId);
    ...
    return {
      store: {
        ...
        vendorRoutingEngine: vendorRouting.fallback,
        vendorRoutingEnginePool: vendorRouting.pool,
        ...deliveryPlanExtras,
        vendorManagesDeliveryDrivers: !!doc.vendorManagesDeliveryDrivers,
        deliveryAssignmentMode: String(
          doc.deliveryAssignmentMode ?? 'SEMI_AUTO',
