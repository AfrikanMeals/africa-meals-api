...
import { mapTrafficExtCacheKey } from '@modules/map-engine-cache/map-engine-cache.keys';
...
export class TrafficExternalProviders {
  ...
  async resolveTomTomFactor(
    ...
    const key = mapTrafficExtCacheKey('tomtom', lat, lng);
    const ttl = this.mapCache?.ttlSec('traffic') ?? 90;
    ...
    if (!this.mapCache) return load();
    return this.mapCache.getOrSetJson<number>(key, ttl, load, {
      engine: 'tomtom',
      kind: 'traffic',
    });
  }
  ...
  async resolveMapboxFactor(
    ...
    const heading =
      Number.isFinite(Number(headingDegrees)) && Number(headingDegrees) >= 0
        ...
        : 0;
    const headingBucket = Math.round(heading / 45) * 45;
    const key = mapTrafficExtCacheKey(`mapbox:${headingBucket}`, lat, lng);
    const ttl = this.mapCache?.ttlSec('traffic') ?? 90;
    ...
    const load = async (): Promise<number | null> => {
      ...
      const toLng =
        lng + (distDeg * Math.sin(rad)) / Math.cos((lat * Math.PI) / 180);
