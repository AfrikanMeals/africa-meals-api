import {
  Body,
  Controller,
  Get,
  Header,
  Put,
  Req,
  UseGuards,
  UsePipes,
  ValidationPipe,
} from '@nestjs/common';
import { ApiBearerAuth, ApiTags } from '@nestjs/swagger';
import { JwtGuard } from '@modules/auth/guards/jwt.guard';
import { UserModel } from '@schemas/user.schema';
import { Request } from 'express';
import { UpdateRecommendationAutomationSettingsDto } from './dto/update-recommendation-automation-settings.dto';
import { RecommendationAutomationSettingsService } from './recommendation-automation-settings.service';

@ApiTags('recommendation-automation-settings')
@Controller('platform/recommendation-automation-settings')
export class RecommendationAutomationSettingsController {
  constructor(
    private readonly _service: RecommendationAutomationSettingsService,
  ) {}

  @Get()
  @Header('Cache-Control', 'no-store, no-cache, must-revalidate')
  getPublic() {
    return this._service.getPublicSettings();
  }

  @ApiBearerAuth('bearer')
  @Put()
  @UseGuards(JwtGuard)
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  update(@Req() req: Request, @Body() body: UpdateRecommendationAutomationSettingsDto) {
    return this._service.updateSettings(req.user as UserModel, body);
  }
}
